...
The canonical Elephas commands are implemented by the following script files in `skills/ocas-elephas/scripts/`:

| Command | Script file | Notes |
|---|---|---|
| `elephas.ingest.journals` | `ingest_journals.py` | Standalone ingestion only. |
| `elephas.consolidate.immediate` | **Implicit in** `cron_ingest_consolidate.py` | No standalone script; the immediate consolidation logic lives inside the combined cron script (see below). |
| `elephas.ingest.memory` | `elephas_deep_pipeline.py` (Phase 1) | Deep consolidation only. |
| `elephas.ingest.sessions` | `elephas_deep_pipeline.py` (Phase 2) | Deep consolidation only. |
| `elephas.consolidate.deep` | `elephas_deep_pipeline.py` (Phase 3) | Deep consolidation only. |
| `elephas.identity.resolve` / `.merge` / `.candidates.*` / `.query` / `.status` / `.init` | Not directly exposed as scripts — use `skill_view` + reasoning or extend pipeline scripts for bulk operations. | Small-scale operations should be done via notebook/REPL against the database. |

### Production cron script

**Use this for scheduled runs** — `cron_ingest_consolidate.py`

The script `skills/ocas-elephas/scripts/cron_ingest_consolidate.py` is the authoritative, production-tested implementation for the `elephas:ingest` cron job. It combines both phases:

1. **Ingestion** — scans all unprocessed journal files, extracts entities from all four `entities_observed` locations plus the Weave enrichment `enriched` field gap, creates Signal → Candidate chains
2. **Immediate consolidation** — promotes high-confidence (`high`/`medium`/`≥0.6`) user-relevant candidates, detects exact-name duplicates, creates nodes with type-correct properties, verifies orphan signal count

**Why this script and not `elephas_pipeline.py`?**
- `elephas_pipeline.py` over-aggressively cleans the ingestion log (removes *all* `signals_created: 0` entries), causing ~900 journal files to be re-processed every cycle and logging duplicate entries
- `elephas_run_ingest_consolidate.py` suffers from a parameter-binding bug that crashes on first signal creation
- `cron_ingest_consolidate.py` is simpler, has the correct stale-entry cleanup logic, and includes the Weave enrichment format-gap bridge by default

**Running manually** (outside the scheduler):
```bash
python3 /root/.hermes/skills/ocas-elephas/scripts/cron_ingest_consolidate.py
```
It will automatically skip already-ingested files and proceed to consolidation.

### Script variants and their status

| Script | Status | Notes |
|---|---|---|
| `cron_ingest_consolidate.py` | ✅ Recommended (immediate) | Single `Database` instance, correct stale-log cleanup, Weave enrichment bridge, exact-name duplicate detection, orphan verification. |
| `elephas_pipeline.py` | ⚠️ Over-aggressive cleanup | Removes *all* `signals_created=0` entries from ingestion log → re-ingests same files repeatedly. Do not use. |
| `elephas_run_ingest_consolidate.py` | ❌ Broken — parameter binding | Crashes with LadybugDB vector-type error on first signal creation. |
| `elephas_pipeline_v2.py` | ⚠️ Incomplete | Intermediate variant, missing Weave enrichment bridge and some edge-case fixes. |
| `elephas_deep_pipeline.py` | ✅ Deep consolidation only | Use for daily deep runs (memory + session ingestion). Contains known `CONTAINS` duplicate-detection bug; verify and manually promote remaining user-relevant candidates after running. |
| `ingest_and_consolidate.py` | ⚠️ Legacy variant | Precedes the critical fixes; prefer `cron_ingest_consolidate.py`. |

### Two-step manual run (if needed)

If you must run the two steps separately for debugging:
1. `python3 skills/ocas-elephas/scripts/ingest_journals.py`
2. `python3 skills/ocas-elephas/scripts/cron_ingest_consolidate.py` (it will skip ingestion and only consolidate)

**Never** run two different pipeline variants back-to-back on the same database — they may use different `Database` instance lifetimes and cause write-visibility issues.
...
**Terminal failure fallback**: If `terminal()` is non-functional (e.g., persistent `FileNotFoundError` for `/tmp` paths), skip file writes and `terminal()` calls entirely. Inline the full pipeline (imports, helpers, DB operations) in a single `execute_code` call with no shell intermediaries. Example structure:
```python
execute_code(code="""
import real_ladybug as lb
from pathlib import Path
...
db = lb.Database(str(DB_PATH))
conn = lb.Connection(db)
# ... full ingestion/consolidation logic ...
""")
```
...
Discovered 2026-04-19 during ingest+consolidation run when attempting incremental pipeline building. Updated 2026-05-04 with terminal failure fallback.
...
| `references/production_scripts.md` | **NEW** — before running any Elephas pipeline script; understands which script to use and which to avoid |
...
- **execute_code import omissions**: When using `execute_code` for journal scanning, config updates, or data processing, always include all required imports (`os`, `json`, `datetime`, `timezone`) at the top of the script. Forgetting to import these modules results in `NameError` and failed executions. Common omitted imports: `os` (for path operations via `expanduser`/`walk`), `json` (for parsing config/ingestion logs), `datetime`/`timezone` (for timestamp handling).
